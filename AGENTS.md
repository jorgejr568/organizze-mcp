...
2. **`account_id` and `credit_card_id` are mutually exclusive on POST /transactions.** If both are present in the body, Organizze silently drops `credit_card_id` and routes the transaction to the bank account. The MCP's service-layer validation rejects the both-set combination up-front. `AccountID` is marshalled with `omitempty` so passing `0` doesn't leak `"account_id":0` and re-trigger the silent drop.
