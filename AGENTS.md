...
- **Lambda runtime env vars:**
  ...
  - Consumer: `STATS_DATABASE_URL` (libpq URI), `STATS_QUEUE_URL` (SQS queue URL), `AWS_REGION`; AWS credentials via the host's IAM role or env-var creds with sqs:ReceiveMessage / DeleteMessage / GetQueueAttributes on the queue ARN. Optional tuning: `STATS_POLL_WORKERS` / `STATS_POLL_BATCH_SIZE` / `STATS_POLL_WAIT_SECONDS` / `STATS_RECEIVE_BACKOFF_SECONDS` / `STATS_PG_POOL_MAX_CONNS` (see `cmd/consumer/README.md` for defaults and ranges).
