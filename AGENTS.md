...
- **MCP-side build-time defaults** (officially-released Docker images only — applies to **both** `organizze-mcp` and `organizze-mcp-oauth`; the two `release.yml` jobs use the same fetch + build-args, so the two binaries report to the same pipeline by default):
...
- **MCP runtime overrides / opt-out:** `MCP_STATS_OPTOUT=1` disables stats entirely; `MCP_STATS_INGEST_URL` / `MCP_STATS_INGEST_TOKEN` override the build-time defaults. Both binaries honour the same env vocabulary.
