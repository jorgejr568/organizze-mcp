...
### Added
- **Consumer throughput knobs.** Two new env vars on `cmd/consumer/`: `CONSUMER_POLLERS` (parallel `ReceiveMessage` loops, default 1) and `CONSUMER_INSERT_CONCURRENCY` (max concurrent inserts per batch, default 1). Both default to current behavior; operators can scale a single container from ~15 msg/s to ~400 msg/s without rebuilding the image. See `cmd/consumer/README.md` for tuning guidance.
...
### Changed
- Consumer now uses a single `DeleteMessageBatch` call per batch instead of N sequential `DeleteMessage` calls. Cuts the per-batch SQS-API time from ~500 ms to ~50 ms (10× improvement on the bookkeeping phase) with no behavior change — at-least-once semantics preserved.
